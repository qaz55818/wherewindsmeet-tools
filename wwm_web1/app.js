        ...
        openAdminBtn.addEventListener('click', () => {
            if (isAdmin) {
                ...
                showToast("已退出管理模式");
                renderRecords(allRecords); // 重新渲染以隱藏 IP 與刪除鍵
            } else {
        ...
        verifyAdminBtn.addEventListener('click', async () => {
            ...
            try {
                ...
                let dbPassword = "admin888"; // 預設防呆密碼
                
                if (adminSnap.exists()) {
                    dbPassword = adminSnap.data().password;
                } else {
                    // 若資料庫內尚無此文件，自動為您建立一筆預設值
                ...
                if (inputPwd === dbPassword) {
                    ...
                    cdTimeInput.value = systemConfig.cdTimeMinutes || 0;
                    renderBannedIps();
                    renderRecords(allRecords); // 重新渲染以顯示 IP 與刪除鍵
